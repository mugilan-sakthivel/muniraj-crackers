...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-40 border-b border-white/[.06] bg-[#090a1d]/80 backdrop-blur-xl">
    <div className="site-container flex min-h-[78px] items-center justify-between gap-4">
      ...
      <Link href="/cart" className="festival-button festival-button-gold shrink-0">My list <span className="ml-1.5 rounded-full bg-black/15 px-1.5 py-0.5" aria-label={`${count} packs selected`}>{count}</span></Link>
    </div>
  </header>
}
